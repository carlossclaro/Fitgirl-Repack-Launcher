document.addEventListener("DOMContentLoaded", async function () {
  const { torrentAPI, primaryAPI, secondaryAPI } = window;
  const dirname = await primaryAPI.getDirname();

  // Start from HTML.
  const myGamesFilePath = primaryAPI.resolvePath(
    dirname,
    "../../src/private/temp/games.json"
  );
  const downloadedGames = primaryAPI.resolvePath(
    dirname,
    "../../src/private/library/downloaded_games.json"
  );
  const infoDownloadedGamesFile = primaryAPI.resolvePath(
    dirname,
    "../../src/private/library/info_downloaded_games.json"
  );
  const autoInstallerPath = primaryAPI.resolvePath(
    dirname,
    "../public/apps/bin/installer.exe"
  );
  ...
  async function updateSingleGameUI() {
    try {
      ...
      // Create an image element for the game image
      const imgElement = document.createElement("img");
      imgElement.src = srcPic;
      imgElement.alt = title;
      imgElement.className = "single-sliding-image";

      // Create a button element for downloading the game
      ...
      //TODO  console.log("Game Screenshots: ", screenshotsContainer);
      ...
      infoContainerGameDesc.innerHTML = gameDesc
        .map((desc) => `<p>${desc}</p>`)
        .join("<br>");

      infoContainer.innerHTML = gameDesc.map((desc) => `<p>${desc}</p>`).join("<br>");



      const spoilerTab = document.createElement("button");
      spoilerTab.className = "spoiler-tab";
      ...
      hiddenGameDesc.innerHTML = gameDesc
        .map((desc) => `<p>${desc}</p>`)
        .join("<br>");
      hiddenGameDesc.style.display = "none";

      const spoilerTabForInfoContainer = document.createElement("button");
      spoilerTabForInfoContainer.className = "spoiler-tab";
      spoilerTabForInfoContainer.textContent = "Click to reveal Game Details";

      const hiddenGameDescForInfoContainer = document.createElement("div");
      hiddenGameDescForInfoContainer.className = "hidden-game-desc";

      hiddenGameDescForInfoContainer.innerHTML = desc;
      hiddenGameDescForInfoContainer.style.display = "none";
      ...
      // Toggle visibility of the hidden content when the spoiler tab is clicked
      spoilerTabForInfoContainer.addEventListener("click", function () {
        if (hiddenGameDescForInfoContainer.style.display === "none") {
          hiddenGameDescForInfoContainer.style.display = "block";
          spoilerTabForInfoContainer.textContent =
            "Click to hide Game Details";
        } else {
          hiddenGameDescForInfoContainer.style.display = "none";
          spoilerTabForInfoContainer.textContent =
            "Click to reveal Game Details";
        }
      });
      ...
      imgElement.addEventListener("contextmenu", async function (event) {
        ...
        let usablePathInstalled = await secondaryAPI.contextMenuLocalInstall(
          title,
          srcPic,
          desc
        );
      ...
      primaryAPI
        ...
        .then((slideData) => {
          ...
          if (
            isDownloading &&
            sessionStorage.getItem("torrentCheckMagnet") === magnetLink
          ) {
      ...
      contentContainer.appendChild(spoilerTabForInfoContainer);
      contentContainer.appendChild(hiddenGameDescForInfoContainer);
      ...
      downloadButton.addEventListener("click", async function () {
        ...
        if (!isDownloading) {
          ...
          try {
            let result = await togglePathWindowAsync(
              magnetLink,
              title,
              srcPic,
              desc,
  ...
  async function checkInstallLib(
    imageGame,
    myTitleGame,
    descriptionGame,
    gameDesc,
    ...
    try {
      ...
      const gameDoneDataStr = await primaryAPI
        .readFile(downloadedGames)
        .catch((error) => console.error("not existing"));
      if (gameDoneDataStr) {
        Object.assign(gameDoneData, JSON.parse(gameDoneDataStr));
      }

      // Read existing infoDownloadedGames if available
      const infoDownloadedGamesStr = await primaryAPI.readFile(
        infoDownloadedGamesFile
      );
      ...
      if (
        isTorrentNameInJSON ||
        (isGameDone === "true" && !isTorrentNameInJSON)
      ) {
        console.log(
          "Game is done. Stopping UI update, stopping torrent, and starting EXE processing."
        );
        stopUIUpdate();
        ...
        const infoDownloadedGamesJson = JSON.stringify(
          infoDownloadedGames,
          null,
          2
        );
        ...
        await Promise.all([
          ...
          primaryAPI.writeFile(
            infoDownloadedGamesFile,
            infoDownloadedGamesJson,
            "utf8"
          ),
        ]);
  ...
  async function updateDownloadUI(
    ...
    document.querySelector(
      "#downloadSpeed"
    ).innerHTML = `Download Speed: ${downloadSpeed}`;
    document.querySelector(
      "#uploadSpeed"
    ).innerHTML = `Upload Speed: ${uploadSpeed}`;
    document.querySelector(
      "#total"
    ).innerHTML = `Total Downloaded : ${totalSize}`;
    document.querySelector(
      "#remaining"
    ...
    document.querySelector(
      "#downloaded"
    ).innerHTML = `Downloaded : ${downloadedSize}`;
  ...
  async function startEXEProcessing() {
    ...
    let gameFolderPathEXE =
      varActualInput + torrentFolderName + "/" + "setup.exe";
  ...
  function startUIDownloadUpdate(titleGame, imageGame, descriptionGame) {
    if (!intervalId) {
      intervalId = setInterval(
        () => updateDownloadUI(titleGame, imageGame, descriptionGame),
        505
      );
    }
  }
   ...
   */
  async function togglePathWindowAsync(
    magnetlink,
    titleGame,
    imageGame,
    descriptionGame
  ...
  function populateImageGrid() {
        ...
        if (this.readyState == 4 && this.status == 200) {
          ...
          $.when.apply($, requests).done(function () {
            var results = postURLs.filter(function (postURL) {
              return getTitleFromUrl(postURL)
                .toUpperCase()
                .replace(/-/g, " ")
     ...
     */
    async function addSlideComponents(
      link,
      title,
      magnetlink,
      descC,
      ...
      primaryAPI
        ...
        .then((slideData) => {
          ...
          try {
            ...
            isUITorrentNameInJSON =
              gameJSON.games && gameJSON.games.includes(forCheckTitle);
